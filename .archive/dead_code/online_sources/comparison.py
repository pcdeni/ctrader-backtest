...
# 1
    ...
    to_date = datetime(2019, 1, 4)
    daily_resolution = False  # run separate optimizations for each day if true, or run it as one long interval
    days_interval = 2
    ...
    deposit = 1000  # starting deposit value
    model = 0  # every tick
    ...
    param = {
         'max_unit': {'value': 1},
         'spread_difference': {'value': 0, 'minimum': 0, 'step': 0.01, 'maximum': 15}, #4?
         'gridstep': {'value': 0.01},
         'stop_loss_limit': {'value': 1, 'minimum': 1, 'step': 1, 'maximum': 1500},
         'angle': {'value': 0.01, 'minimum': 0.000001, 'step': 0.000001, 'maximum': 0.001},
         'frequency': {'value': 1, 'minimum': 0.01, 'step': 0.01, 'maximum': 1},
         ##'frequency': {'value': 1, 'minimum': 1, 'step': 1, 'maximum': 100},
         'steepness': {'value': 1, 'minimum': 1, 'step': 0.1, 'maximum': 20},
         'phase': {'value': 0, 'minimum': -1*180, 'step': 1, 'maximum': 180},
         'DC': {'value': 0.1, 'minimum': 0, 'step': 0.01, 'maximum': 1},
         ##'puffer': {'value': 0.1, 'minimum': 0, 'step': 0.01, 'maximum': 1},
         'puffer': {'value': 1},
         ##'dummy': {'value': 1},
         ##'delimeter': {'value': ""},
        ...
        with open(param_file, 'w') as fp:
            ...
            for j in values1:
                ...
                if j == "output_file_location_and_name":
                    opt = 'N'
                    fp.write('{}={}_{}_{}_{}.csv\n'.format(j, ea_name, day[0].strftime('%Y%m%d'), day[1].strftime('%Y%m%d'),sd))
...
#2
    ...
    # specify the name of the expert advisor
    ea_name = 'static_grid_with_direction_no_modify'
...
#3
    ...
    # specify the name of the expert advisor
    ea_name = 'static_grid_without_direction_modify'
...
#4
    ...
    # specify the name of the expert advisor
    ea_name = 'static_grid_without_direction_no_modify'
